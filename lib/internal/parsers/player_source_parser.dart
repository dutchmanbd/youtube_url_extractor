...
class PlayerSourceParser {
  ...
  List<BaseCipherOperation> parseCipherOperations() {
    ...
    if (entryPointBody == null) {
      throw ParseException(
          message: 'Could not find the signature decipherer function body.');
    }
    ...
    List<BaseCipherOperation> operations = [];
    ...
    for (var i = 0; i < entryPointLines.length; i++) {
      ...
      if (calledFuncName == charSwapFuncName) {
        // Swap operation
        var index = int.tryParse(RegExp(r"\(\w+,(\d+)\)")
                    .firstMatch(entryPointLines[i])
                    ?.group(1) ??
                "0") ??
            0;
